...
public class SarsaLamdba_Agent {
    ...
    public static void main(String[] args) {
        ...
        SarsaLamdba_Agent agent = new SarsaLamdba_Agent();
        agent.train(SarsaLamdba_Agent.EPISODES);
    ...
    public void train(int numEpisodes) {
        ...
        double[] z = new double[TOTAL_FEATURES];

        for (int episode = 0; episode < numEpisodes; episode++) {

            // Resetting the elegibility traces.
            Arrays.fill(z, 0.0);

            // Resetting the environment. New episode will start.
            double[] s = game.randomReset();

            // Choosing the first action.
            int a = chooseAction(computeAllQ(s));
            ...
            while (!done) {
                ...
                // Executing the action taken.
                double[] nextState = game.step(ACTIONS[a]);
                done = nextState[0] == 1;
                double reward = nextState[1];

                // Getting the active tiles.
                int[] activeTiles = getActiveTileIndices(s, a);

                // Decaying the eligibility traces.
                for (int i = 0; i < TOTAL_FEATURES; i++) {
                    z[i] *= (GAMMA * LAMBDA);
                }

                // Updating the eligibility traces for active tiles.
                ...
                double delta;
                if (done) {
                    delta = reward - computeQ(s, a);
                } else {

                    double[] qNext = computeAllQ(nextState);
                    int nextA = chooseAction(qNext);
                    delta = reward + GAMMA * computeQ(nextState, nextA) - computeQ(s, a);
                ...
                for (int idx = 0; idx < TOTAL_FEATURES; idx++) {
